...
int32_t DatasetView::get_label(size_t sample) const {
    return dataset->get_label(indices[sample]);
}
...
const std::vector<int32_t>& DatasetView::get_labels() const {
    return dataset->get_labels();
}

double DatasetView::get_value(size_t sample, size_t feature) const {
    return dataset->get_value(indices[sample], feature);
}

const std::vector<size_t>&
DatasetView::get_feature_order(size_t feature) const {
    return feature_order[feature];
}

void DatasetView::construct_feature_order() {
    feature_order.resize(get_n_features(),
                         std::vector<size_t>(get_n_samples()));
    ...
    for(size_t i = 0; i < get_n_features(); i++) {
        std::sort(feature_order[i].begin(), feature_order[i].end(),
                  [&i, this](const size_t& a, const size_t& b) {
                      return this->get_value(a, i) < this->get_value(b, i);
                  });
    }
}
